...
<template>
  ...
  <div class="pokemon-card elevation-3 mt-2" :class="pokemon.type">
    <router-link class="black" :to="'/detail?id=' + pokemon.number">
    <v-container>

      <v-row align="center">
        <v-col cols="1" align="center">
          <img class="pokemon-icon" :class="[pokemon.type]" :src="getIconAssetUrl(pokemon.type)" />
        </v-col>

        <v-col cols="3">
          <p class="font-weight-medium text-center text-capitalize black">#{{ paddingZeroLeft(pokemon.number, 4) }}</p>
        </v-col>

        <v-col cols="5">
          <p class="font-weight-medium text-capitalize black">{{ pokemon.name }}</p>
        </v-col>
